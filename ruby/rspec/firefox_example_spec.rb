...
=end
  ...
  before(:each) do |test|
    caps = {
        ...
        # w3c is enabled by default in Firefox in version 53 or higher,
        # so there is no need to explicitly set the w3c setting to "moz:firefoxOptions" => {w3c: true },
...
=begin
    ...
    in this case we're checking to see if our test in SauceLabs session can open Firefox using W3C, then check the title page.
=end
